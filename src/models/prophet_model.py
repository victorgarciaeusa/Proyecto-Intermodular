...
def grid_search_prophet(train_df):
    ...
    for i, combo in enumerate(combos):
        ...
        try:
            ...
            m.add_seasonality(name="weekly", period=7, fourier_order=params["fourier_order_weekly"])
            ...
            df_cv = cross_validation(
                m, initial="180 days", period="30 days", horizon="28 days", disable_tqdm=True
            )
    ...
    pd.DataFrame(results).sort_values("mape_cv").to_csv(
        "/tmp/prophet_grid_results.csv", index=False
    )
...
def run_prophet():
    ...
    for sku in sorted(df["sku"].unique()):
        ...
        for i in range(MIN_TRAIN, len(grp_p) - 1, 3):  # paso 3
            ...
            try:
                ...
                m.add_seasonality(
                    name="weekly", period=7, fourier_order=best_params["fourier_order_weekly"]
                )
                m.add_regressor("promotion_flag", standardize=False)
                m.add_country_holidays(country_name="ES")
    ...
    mape_promo = mape(
        results[results["promotion_flag"] == 1]["y_true"],
        results[results["promotion_flag"] == 1]["y_pred"],
    )
    mape_nopromo = mape(
        results[results["promotion_flag"] == 0]["y_true"],
        results[results["promotion_flag"] == 0]["y_pred"],
    )
    mape_sku = pd.DataFrame(list(mape_per_sku.items()), columns=["sku", "mape"]).sort_values("sku")
