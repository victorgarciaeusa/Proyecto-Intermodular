...
XGB_PARAMS = {
    "n_estimators": 1050, "max_depth": 4,
    "learning_rate": 0.006508, "subsample": 0.7124,
    "colsample_bytree": 0.7665, "colsample_bylevel": 0.7336,
    "min_child_weight": 15, "gamma": 0.0401,
    "reg_alpha": 0.1552, "reg_lambda": 19.676,
...
def predict_sarimax(sku, grp, horizon_days):
    ...
    try:
        ...
        forecast = fit.get_forecast(steps=horizon_days,
                                    exog=future[["promotion_flag"]].astype(float))
...
def add_xgb_features(df):
    ...
    for w in [7, 14, 21, 28]:
        df[f"ma_{w}"]  = df.groupby("sku")["quantity"].transform(lambda x: x.shift(1).rolling(w).mean())
        df[f"std_{w}"] = df.groupby("sku")["quantity"].transform(lambda x: x.shift(1).rolling(w).std())
    df["rolling_max_7"]   = df.groupby("sku")["quantity"].transform(lambda x: x.shift(1).rolling(7).max())
    df["rolling_min_7"]   = df.groupby("sku")["quantity"].transform(lambda x: x.shift(1).rolling(7).min())
    ...
    df["season_encoded"]  = df["sale_date"].dt.month.map(
        lambda m: season_map["winter"] if m in [12,1,2]
        else season_map["spring"] if m in [3,4,5]
        else season_map["summer"] if m in [6,7,8]
        else season_map["autumn"]
    )
    import holidays
...
XGB_FEATURE_COLS = [
    "quantity_lag_1","quantity_lag_7","quantity_lag_14","quantity_lag_21","quantity_lag_28",
    "ma_7","ma_14","ma_21","ma_28","std_7","std_14","std_21","std_28",
    "rolling_max_7","rolling_min_7","rolling_range_7","lag7_vs_ma28","lag1_vs_ma7",
    "day_of_week","day_of_month","week_of_year","month","quarter",
    "season_encoded","is_weekend","is_holiday","is_month_end","is_month_start",
    "promotion_flag","sku_encoded"
]
    ...
    try:
        ...
        for i in range(horizon_days):
            ...
            if last_row.isnull().any().any():
                pred = history["quantity"].iloc[-7] if len(history) >= 7 else history["quantity"].mean()
            else:
...
def validate_predictions(preds, hist_mean, hist_std, sku):
    ...
    for i, p in enumerate(preds):
        ...
        if p > hist_mean + 3 * hist_std:
            alerts.append(f"SKU {sku} día {i+1}: predicción outlier ({p:.2f} > mean+3std={hist_mean+3*hist_std:.2f})")
        if p < hist_mean - 3 * hist_std:
            alerts.append(f"SKU {sku} día {i+1}: predicción outlier ({p:.2f} < mean-3std={hist_mean-3*hist_std:.2f})")
    return alerts
...
def detect_data_drift(engine, df):
    ...
    drift_qty   = abs(recent["quantity"].mean() - historic["quantity"].mean()) / (historic["quantity"].mean() + 1e-9)
...
def compute_mape_rolling(engine, window_days=30):
    ...
    try:
        ...
        for model_name, grp in df.groupby("model_name"):
            ...
            if mask.sum() == 0:
                continue
            mape = np.mean(np.abs((grp[mask]["actual"] - grp[mask]["predicted_qty"]) /
                                   grp[mask]["actual"])) * 100
...
def run_batch():
    ...
    all_predictions = []
    run_metrics     = {
        "skus_ok": 0, "skus_error": 0,
        "total_predictions": 0, "alerts": [],
        "sku_timing": {}
    }
    ...
    for sku, grp in df_curated.groupby("sku"):
        ...
        if sarimax_preds is not None and xgb_preds is not None:
            ensemble_preds = (ENSEMBLE_WEIGHTS["sarimax"] * sarimax_preds +
    ...
    if all_predictions:
        ...
        df_preds.to_sql("predictions", engine, schema="analytics",
