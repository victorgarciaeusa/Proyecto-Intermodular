    ...
    try:
        ...
        for model_name, grp in df.groupby("model_name"):
            ...
            if mask.sum() == 0:
                continue
            mape = np.mean(np.abs(
                (grp[mask]["actual"] - grp[mask]["predicted_qty"]) / grp[mask]["actual"]
            )) * 100
            needs_retrain = mape > MAPE_BASELINE * MAPE_REENTRAIN_RATIO
            result[model_name] = {
                "mape_rolling": round(mape, 4),
...
def retrain_models(mape_result: dict, drift_result: dict) -> bool:
    ...
    needs_retrain = (
        drift_result.get("drift_qty_alert", False) or
        drift_result.get("drift_promo_alert", False) or
        any(v.get("needs_retrain", False)
            for v in mape_result.values() if isinstance(v, dict))
    )
...
def generate_report(drift_result: dict, mape_result: dict,
                    batch_ok: bool, retrained: bool) -> str:
    logger  = get_run_logger()
    report  = {
        "timestamp":    datetime.utcnow().isoformat(),
