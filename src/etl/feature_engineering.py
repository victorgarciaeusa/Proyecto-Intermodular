...
def build_features():
    ...
    logging.info(f"[FEATURES] Filas tras dropna lags: {len(df)} (eliminadas: {rows_before - len(df)})")
    ...
    logging.info(f"[FEATURES] Rango: {df['sale_date'].min().date()} -> {df['sale_date'].max().date()}")
